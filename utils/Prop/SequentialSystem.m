classdef SequentialSystem < Prop
    ...
    methods
        ...
        function init(obj, Before_Mesh, After_Mesh)
            ...
            obj.Layers(1).init(Before_Mesh, obj.Layers(2).input_mesh());
            obj.Layers(end).init(obj.Layers(end-1).output_mesh(), After_Mesh);
            for iter=2:length(obj.Layers)-1
                obj.Layers(iter).init(obj.Layers(iter-1).output_mesh, obj.Layers(iter+1).input_mesh());
            end
        end
